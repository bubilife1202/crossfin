...
- `GET /api/premium/arbitrage/kimchi` route spread index (normally $0.05, currently free in open beta)
- `GET /api/premium/arbitrage/opportunities` arbitrage decision service — POSITIVE_SPREAD/NEUTRAL/NEGATIVE_SPREAD (normally $0.10, currently free in open beta)
- `GET /api/premium/bithumb/orderbook?pair=BTC` orderbook (normally $0.02, currently free in open beta)
- `GET /api/premium/market/korea` market sentiment (normally $0.03, currently free in open beta)
- `GET /api/premium/enterprise` enterprise receipt endpoint (normally $20.00, currently free in open beta)
